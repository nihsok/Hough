...
function calc_eigen(){
  //remove old event listenter on table, and remove plot?
  ...
  for (let i=1;i<=15;i++){
    ...
    x1[1].cells[i+2].innerHTML = (4*Math.PI*7/Math.sqrt(8000/(lambda[len_sym-i]*ev2ed)-1)).toFixed(1)
    ...
    x1[4].cells[i+2].innerText = vector[len_sym-i].vector._data[0].toExponential(2)
    x1[5].cells[i  ].innerText = vector[len_sym-i].vector._data[1].toExponential(2)
    x1[6].cells[i  ].innerText = vector[len_sym-i].vector._data[2].toExponential(2)
    x1[7].cells[i  ].innerText = vector[len_sym-i].vector._data[3].toExponential(2)
    x1[8].cells[i  ].innerText = vector[len_sym-i].vector._data[4].toExponential(2)
...
function plot_hough(s,flag_asymmetric,coef){
  ...
  for (let i=flag_asymmetric;i<flag_asymmetric+coef.length;i++){
    const start = (s*m_max+2*i)*nlat

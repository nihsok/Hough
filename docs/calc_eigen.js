...
function calc_eigen(){
  ...
  for (let r=s;r<=n;r++){ //r=s+i
    L[r-s] = Math.sqrt((r+s+1)*(r+s+2)*(r-s+1)*(r-s+2))/( (2*r+3)*Math.sqrt((2*r+2)*(2*r+5))*(s/sigma-(r+1)*(r+2)) )
  ...
  for (i=1;i<=10;i++){
    x1[0].cells[i+3].innerText =(lambda[len_sym-i]*ev2ed).toFixed(3)
    x1[1].cells[i+3].innerText = lambda[len_sym-i].toFixed(3)
    x1[2].cells[i+3].innerText = vector[len_sym-i].vector._data[0].toFixed(3)
    x1[3].cells[i+1].innerText = vector[len_sym-i].vector._data[1].toFixed(3)
    x1[4].cells[i+1].innerText = vector[len_sym-i].vector._data[2].toFixed(3)
    x1[5].cells[i+1].innerText = vector[len_sym-i].vector._data[3].toFixed(3)
